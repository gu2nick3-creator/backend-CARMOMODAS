...
app.post("/api/checkout", async (req, res) => {
  ...
  if (!accessToken) return res.status(500).json({ error: "mp_access_token_missing" });
  ...
  try {
    ...
    return res.status(500).json({ error: "mp_preference_failed" });
  }
});
...
// FRONT no final (sem engolir /api)
